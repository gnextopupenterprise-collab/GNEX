(() => {
  ...
  window.subscribeGnexUserPush=async function(){
    ...
    const registration=await navigator.serviceWorker.register("scrim-sw.js?v=23",{updateViaCache:"none"});
...
window.openDepartmentChat = async function(department){
  ...
  // code lama sambung...
      ...
      state.department =
        department;
      if($("#gc-topup-now"))$("#gc-topup-now").hidden=department!=="topup";
  ...
  window.openPinTopupForm=function(){
    if(state.department!=="topup")return;
    const modal=$("#gc-pin-order-modal");if(!modal)return;
    modal.hidden=false;modal.setAttribute("aria-hidden","false");
    setTimeout(()=>$("#gc-pin-game")?.focus({preventScroll:true}),50);
  };
